<div class="h-screen overflow-y-auto no-scrollbar">
    @php
        $isUnlocked = session()->has('is_unlocked_' . $folder->slug) && session('is_unlocked_' . $folder->slug);
    @endphp

    @if (empty($folder->password) || $isUnlocked)
        ...
        <div class="mb-16">
            @livewire('task', ['tasks' => $tasks])
        </div>
    @else
        @livewire('auth-folder', ['folder' => $folder])
    @endif
</div>

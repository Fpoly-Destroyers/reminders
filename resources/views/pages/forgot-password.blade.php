...
<div class="flex items-center justify-center h-screen text-gray-600 bg-gray-50">
    <div class="py-16 px-48 bg-white shadow rounded min-h-[600px] flex flex-col justify-center">
        ...
        @endsession
        @session('success')
        <div class="rounded border-1 border-blue-700 bg-blue-50 px-4 py-2 text-xs mb-4 text-blue-500 break-words w-64">
            {{ session('success') }}
        </div>
        @endsession
        ...
        <form action="{{route('post.forgot-password')}}" method="post">
            ...
            @method('POST')
            <div class="mb-4">
                <input placeholder="Email" type="email" id="email" name="email" value="{{ old('email') }}" class="bg-gray-50 border border-gray-300 text-gray-900 text-xs rounded focus:ring-blue-500 focus:border-blue-500 block w-64 p-1.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" />
                @error('email')

            ...
            <thead>
                <tr>
                    @foreach ($weekDays as $day)
                        <th>
                            <div class="ps-2 pe-3 py-2 w-full flex justify-center">
                                <p class="font-medium text-center text-gray-800 dark:text-gray-100">
                                    {{ $day }}
                                </p>
                            </div>
                        </th>
                            ...
                            @if ($dayNumber > 0 && $dayNumber <= $daysInMonth)
                                <td>
                                    <a href="{{ route('pickDate', [$changeYear, $changeMonth, $dayNumber]) }}"
                                        ...
                                        <div
                                            class="mb-2 px-1 py-2 cursor-pointer hover:bg-gray-200 flex justify-center items-center w-full rounded-lg cursor-pointer @if ($dayNumber == $currentDay && $currentMonth == $changeMonth && $currentYear == $changeYear) focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-700 focus:bg-indigo-500 hover:bg-indigo-500 w-9 h-9 flex items-center justify-center bg-indigo-700 rounded-lg @endif">
                                            <p
                                            ...
                                            <span class="text-red-500 dark:text-red-100 ps-1">•</span>
                                        </div>
                                    </a>
                                </td>
                            @else
                                <td>
                                    <div
                                        class="px-1 py-2 cursor-pointer flex w-full justify-center hover:bg-gray-200 rounded-lg">
